...
$cat_days=SQLSelect("select clnd_events.*,day(DUE) as DAY,month(DUE) as MON,clnd_categories.CALENDAR_COLOR from clnd_events inner join clnd_categories on clnd_events.calendar_category_id=clnd_categories.id where " . $mon_filt . "clnd_categories.AT_CALENDAR=1 and  year(DUE)=" . $year . "  and is_nodate=0"); //lsv added is_nodate 20201016
...
//		$cat_days=SQLSelect("select * from clnd_events where " . $filtr . "(Day(DUE)=" . $list_day . " and month(DUE)=" . $month . " and (year(DUE)=" . $year . " or (IS_REPEATING=1 and REPEAT_TYPE=1)))");
